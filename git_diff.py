...
def create_prompt(diff_content, file_changes):
    return f"""
...
def main():
    ...
    to_clipboard = "--clipboard" in sys.argv or "-c" in sys.argv
    ...
    if to_clipboard:
        ...
        promtp = create_prompt(diff_content, file_changes)
        pyperclip.copy(promtp)
        logger.info(
            f"""
        產生提示詞\n{promtp}
        """
        )
        logger.info("差异内容已复制到剪贴板")
        return
